...
object Screen {
    ...
    fun findStarRows(b: Bitmap): IntArray {
        ...
        // ── ②-a 기준점을 **잔차 평균**으로 다듬는다 ──
        // ⚠️ 아래에서 '가장 센 자리'를 찍으면 안 된다. 그 자리는 촬영마다 몇 px씩 흔들리는데,
        //    실측상 **4px만 어긋나도** 같은 줄의 지문 점수가 0.76 으로 떨어져 '다른 줄'로 보인다
        //    (0.90 기준). 그러면 같은 줄이 두 번 담겨 이름과 그림의 짝이 통째로 밀린다.
        // ⚠️ **격자점끼리 평균 내도 안 된다** — 개수가 짝수면 격자 사이에 떨어진다.
        //    기준 하나를 잡고 **어긋난 양만** 평균 내야 항상 격자 위에 있다.
    ...
    /**
     * 그 별줄의 카드가 **화면에 온전히 보이나.**
     *
     * 스크롤은 아무 데서나 멈추므로, 어떤 줄은 위쪽 머리(프리셋 탭·팀 진열대)에 반쯤 가리거나
     * 아래쪽 바에 걸린다. 그런 줄에서 지문을 뜨면 **카드가 아니라 잘린 화면**을 뜨게 되고,
     * 그 다섯 마리는 나중에 영영 못 알아본다.
     *
     * 실측(편집 모드): 카드는 별줄 기준 **−229 ~ +61**. 격자가 쓸 수 있는 높이는 1060~2790.
     * → 별줄이 **1300~2720** 안에 있어야 카드가 다 보인다.
     *
     * 행은 아래에서 들어와 위로 올라가고 한 번에 2행(634px)씩만 미므로,
